...
function QPopup(ele, options) {
  this.opt = {
    ...
    inAnimit: 'qt-popUp',                  //进入动画的class
    outAnimit: 'zoomOut',                  //消失动画的class
    innerScroll: false,                    //是否进行对内滚动的特殊支持，会设置<html>的高度与overflow: hidden;会导致正常布局的页面滚回顶部(建议可尝试使页面内容在与anchor同级元素中内滚动，来避免出现弹窗的滚动事件引起全局滚动)
    ...
    events: {
      //'tap .invite-weixin': 'inviteWeixin' //用例
    },
    onReady: function() {
    },
    onShow: function() {
...
QPopup.prototype = {
  ...
  buileFrame: function(opt) {
    var defaultTpl =
      ...
      '<div class="qt-popup">' +
      '<div class="qt-popup-title">温馨提示：</div>' +
      '<div class="qt-popup-content"> ' +
      '<div class="qt-popup-msg"></div> ' +
      '</div>             ' +
  ...
  show: function(callBack, opt) {   //在show 时可以通过传入opt重新设置参数
    ...
    if (me.opt.noAnimit) {
      ...
      $qtPopup.one($.fx.animationEnd, function() {
        ...
        $qtPopupMask.removeClass('qt-fadeIn');
        showedAction();
      })
      me.showing = true;
      $qtPopup.addClass(this.opt.inAnimit);
      $qtPopupWrap.css({'display': 'block'});
      $qtPopupMask.css({'display': 'block'}).addClass('qt-fadeIn');
  ...
  hide: function(callBack) {
    ...
    if (me.opt.noAnimit) {
      ...
      $qtPopup.one($.fx.animationEnd, function() {
        $qtPopupWrap.css({'display': 'none'});
        $qtPopupMask.css({'display': 'none'}).removeClass('qt-fadeOut');
      ...
      $qtPopupMask.addClass('qt-fadeOut');
      $qtPopup.css({'-webkit-animation-duration': animitTime + 's'});
      $qtPopupMask.css({'-webkit-animation-duration': animitTime + 's'});
    }
